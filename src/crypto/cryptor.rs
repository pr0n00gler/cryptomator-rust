...
use crate::crypto::error::CryptoError::{InvalidFileHeaderLength, InvalidFileChunkLength};
...
const FILE_HEADER_LENGTH: usize = FILE_HEADER_NONCE_LENGTH + FILE_HEADER_PAYLOAD_LENGTH + FILE_HEADER_MAC_LENGTH;
...
const FILE_CHUNK_LENGTH: usize = FILE_CHUNK_CONTENT_NONCE_LENGTH + FILE_CHUNK_CONTENT_PAYLOAD_LENGTH +
...
impl Cryptor {
    ...
    pub fn decrypt_file_header(&self, mut encrypted_header: Vec<u8>) -> Result<FileHeader, CryptoError> {
        if encrypted_header.len() < FILE_HEADER_LENGTH {
            return Err(InvalidFileHeaderLength(String::from(
                format!("file header must be exactly {} bytes length, got: {}", FILE_HEADER_LENGTH ,encrypted_header.len())
            )));
        }
    ...
    pub fn decrypt_content<R: Read, W: Write>(&self, mut input: R, mut output: W) -> Result<(), CryptoError> {
        ...
        loop {
            ...
            let chunk_content = self.decrypt_chunk(file_header.nonce.as_ref(),
                                                   file_header.payload.content_key.as_ref(), chunk_number,
    ...
    pub fn decrypt_chunk(&self, header_nonce: &[u8], file_key: &[u8], chunk_number: usize,
        ...
        if encrypted_chunk.len() < FILE_CHUNK_CONTENT_MAC_LENGTH + FILE_CHUNK_CONTENT_NONCE_LENGTH {
            return Err(InvalidFileChunkLength(String::from(
                format!("file chunk must be more than {} bytes length, got: {}", FILE_CHUNK_CONTENT_MAC_LENGTH + FILE_CHUNK_CONTENT_NONCE_LENGTH, encrypted_chunk.len())
            )));
        }
