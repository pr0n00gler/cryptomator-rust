...
fn main() {
    ...
    let decrypted_filename = cryptor.decrypt_filename("fXQEfw6iSwP1esHbRznuVFZqv_LQFqNwC2r2LOQa-A==", b"").unwrap();
    ...
    let decrypted_file = fs::File::create(String::from_utf8(decrypted_filename).unwrap()).unwrap();
    cryptor.decrypt_content(encrypted_file, decrypted_file).unwrap();
}