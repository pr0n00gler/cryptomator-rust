...
use cryptomator::providers::{DropboxFS, LocalFs};
...
async fn main() {
    ...
    match opts.subcmd {
        Command::Create(c) => match opts.filesystem_provider {
            ...
            FilesystemProvider::Dropbox => {
                let auth = dropbox_sdk::oauth2::get_auth_from_env_or_prompt();
                let client = UserAuthDefaultClient::new(auth);

                create_command(
                    DropboxFS::new(Arc::new(Mutex::new(client))),
                    &vault_path,
                    &full_storage_path,
                    c,
                )
        ...
        Command::MigrateV7ToV8 => match opts.filesystem_provider {
            ...
            FilesystemProvider::Dropbox => {
                ...
                migrate_v7_to_v8_command(DropboxFS::new(Arc::new(Mutex::new(client))), &vault_path)
            }
        },
